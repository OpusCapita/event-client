...
```JS
const MqEvents = require('@opuscapita/event-client');

var events = new EventClient({ consul : { host : '{{your-consul-host}}' } });

// Subscribe to a channel by name.

events.subscribe((message, rawMessage) => {
    console.log(message); // displays parsed JSON message
    console.log(rawMessage); // displays original message
}, 'my-channel');
