...
class AmqpChannel
{
    ...
    async close()
    {
        ...
        if (this.channel) {
            try {
                ...
                this.logger.error('AmqpChannel#close: Caught exception from call to this#clearConsumers. ', e);
            }

            try {
                await retry(() => this.channel.close(), {
                    'max_tries': 1,
                    timeout: 2000
                });
            } catch (e) {
                this.logger.error('AmqpChannel#close: Caught exception from call to this.channel#close (amqplib). ', e);
            }
        }
    ...
    async nackMessage(message, allUpTo = false, requeue = true)
    {
        const channel = await this.getChannel();
    ...
    async createAndBindQueue({queueName, exchangeName, topic})
    {
        ...
        try {
            ...
            this.logger.error('AmqpChannel#createAndBindQueue: Caught exception from call to amqplib channel#assertQueue. ', e);
            throw e; // Rethrow to not mess with the behaviour before introducing the above try..catch
        }
        ...
        try {
            ...
            this.logger.error('AmqpChannel#createAndBindQueue: Caught timeout exception from bluebird-retry call  this.channel#bindQueue. ', e);
            throw e; // Rethrow to not mess with the behaviour before introducing the above try..catch
        }
    }
    ...
    {
        this.logger.info(`AmqpChannel#removeConsumer: Trying to remove consumer for topic "${topic}."`);
        ...
        {
            try {
                await retry(() => this.channel.cancel(consumer.consumerTag), {
                    'max_tries': 1,
                    timeout: 1500
                });
            } catch (e) {
                this.logger.error('AmqpChannel#removeConsumer: Failed to remove consumer gracefully. ', e);
            }
   ...
   */
    doPublishWait(message) {
        this.logger.log('AmqpChannel#doPublishWait: Putting message to wait queue', message);
    ...
    async queueExists(queueName)
    {
        ...
        {
            this.logger.error('AmqpChannel#queueExists: Failed to call checkQueue  with exception. ', e);
            exists = false;
        }
        finally
        {
            try {
                ...
                this.logger.error('AmqpChannel#queueExists: Failed to close temporary channel with exception. ', e);
            }
        }
    ...
    {
        if (!this.channel) {
            if (!this.isClosing) this.initAmqpChannel();
    ...
    async getNewChannel()
    {
        ...
        channel.on('error', this.onChannelError.bind(this));
        channel.on('close', this.onChannelClose.bind(this));
        channel.on('drain', this.onChannelDrain.bind(this));
        channel.on('return', this.onChannelReturn.bind(this));

        this.logger.info('AmqpChannel#getNewChannel: Channel created.');

        return channel;
    }

    async flushWaitQueue() {
        if (this.useWaitQueue && this.state === AmqpChannel.CS_BLOCKED) {
    ...
    /**
     * Handler for channel error events.
     *
     * @function onChannelError
     * @param {Error} err Exception that was generated inside amqplib
     * @param {number} err.code - Status code of the exception, eg. 404 (Queue not found)
     */
    onChannelError(err) {
    ...
    onChannelClose(err) {
        this.logger.info('AmqpChannel#onChannelClose: onChannelClose handler was triggered on channel with state ', this.state, ' and err: ', err);

        this.channel = null; // Not using AmqpChannel#close() here because this would also clear the subscriptions.
        this.waitingCounter = 0; // Messages pending on the Socket have been deleted

        // FIXME Trigger creation of new channel if not called by this#close()
    ...
    onChannelDrain() {
        ...
        if (this.state === AmqpChannel.CS_READY) {
            this.useWaitQueue = false;
    ...
    async _registerMissingConsumers()
    {
        ...
        const getQueueDataPromises = consumers.map((c) => {
            return this._getQueueData(c.queueName)
                .then((res) => res.consumer_details.map(d => d.consumer_tag))
                .catch(e => {
                    this.logger.error('AmqpChannel#_registerMissingConsumers: Exception caught from call to this#_getQueueData. ', e);
                    return [];
                });
        });

        const activeTags = [].concat(...(await Promise.all(getQueueDataPromises)));
        ...
        if (missingConsumers.length > 0)
        {
            ...
            this.logger.info('AmqpChannel#_registerMissingConsumers: Active consumers "%j".', activeTags);
            this.logger.info('AmqpChannel#_registerMissingConsumers: Missing consumers "%j".', missingConsumers);
        }
        ...
        result = await Promise.all(missingConsumers.map(async (c) =>
        {
            ...
            try {
                ...
                this.logger.error('AmqpChannel#_registerMissingConsumers: Exception caught from call to this#removeConsumer. ', e);
            }

            return this.registerConsumer(c)
                .catch((e) => this.logger.error('AmqpChannel#_registerMissingConsumers: Exception caught from call to this#registerConsumer. ', e));
        }));

        if (missingConsumers.length > 0) {
            this.logger.info('AmqpChannel#_registerMissingConsumers: Missing consumers registration done.');
        }
    ...
    _registerConnectionListeners()
    {
        this.connection.events.on('connection_blocked', () => {
            this.useWaitQueue = true;
            this.state = AmqpChannel.CS_BLOCKED;
        });
        ...
        this.connection.events.on('connected', async () => {
            try {
                ...
                if (this.consumers.size === 0 && this.config.allowPing) {
                ...
                this.logger.error('AmqpChannel: failed to create a new channel after connected.', e);
                this.state = AmqpChannel.CS_ERROR;
            }
