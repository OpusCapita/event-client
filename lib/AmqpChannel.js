...
class AmqpChannel
{
    constructor(connection, logger, connectionConfig, config)
    {
        ...
        this.consumers = new Map(); // TODO As every subscriber gets its own channel -> why a Map?
        ...
        try
        {
            ...
            if (this.config && this.config.useConfirmChannel) {
                ...
                result = new Promise((resolve, reject) => {
                    // TODO check channel state before sending
                    channel.publish(exchangeName, topic, messageBuffer, options, (err) => {
                        ...
                        if (err) {
                            ...
                            this.unackedMsgs.set(msgKey, {...this.unackedMsgs.get(msgKey), state: 2}); // Update the state to 'failed'

                            reject(err);
                        } else {
                            this.unackedMsgs.get(msgKey) && this.unackedMsgs.delete(msgKey);
                            resolve(true);
                        }
                    });
                });
