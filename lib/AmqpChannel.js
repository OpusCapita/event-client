...
class AmqpChannel
{
    constructor(connection, logger)
    {
        ...
        this.connection.events.on('error', async () =>
        {
            ...
            await this.connection.reconnect().catch(e => this.logger.error('Reconnect failed: ', e));
    ...
    async reRegisterConsumers()
    {
        try
        {
            if(this.connection)
            {
                this.logger.info('Starting re-registration of consumers...', Object.keys(consumers));

                const consumers = { ...this.consumers };
                Object.keys(consumers).map(key => { delete this.consumers[key]; delete overallConsumers[key]; });
                this.channel = null;
