...
class AmqpChannel
{
    ...
    async publish({exchangeName, topic, messageBuffer, options})
    {
            ...
            if (this.config && this.config.useConfirmChannel) {
                result = new Promise((resolve, reject) => {
                    channel.publish(exchangeName, topic, messageBuffer, options, (err) => {
                        ...
                        if (err) {
                            this.logger.error('AmqpChannel: message got n-acked by message broker.');
                            this.unackedMsgs.has(msgKey) && this.unackedMsgs.set(msgKey, {...this.unackedMsgs.get(msgKey), state: this.MSG_NACKED}); // Update the state to 'failed'
                 ...
                 */
                this.unackedMsgs.set(msgKey, {...this.unackedMsgs.get(msgKey), state: this.MSG_SENT});
        ...
        {
            // This should only happen when #getChannel() throws. In that case something is seriously
            // broken and we should write the message to logstash.
            //
            // TODO

            this.unackedMsgs.has(msgKey) && this.unackedMsgs.set(msgKey, {...this.unackedMsgs.get(msgKey), state: this.MSG_FAILED}); // Update the state to 'failed'
            this.logger.error(e);
        };
