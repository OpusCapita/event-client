...
class AmqpChannel
{
    constructor(connection, logger, connectionConfig)
    {
        ...
        this.missingConsumersWorker = setInterval(() => this._registerMissingConsumers(), 15000);
    ...
    {
        const topics = Object.keys(this.consumers);
        const consumers = Object.values(this.consumers);

        const activeTags = [ ].concat(...(await Promise.all(topics.map(t => this._getQueueData(t).then(res => res.body.consumer_details.map(d => d.consumer_tag)).catch(e => [ ])))));
