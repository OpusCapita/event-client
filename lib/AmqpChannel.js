...
class AmqpChannel
{
    ...
    async doPublishConfirm(message, [msgKey, cacheEntry]) {
        ...
        let deliveryTimeout = setTimeout(async () => {
            // TODO if connection changed to blocked, move message to waitQueue
        ...
        return new Promise(async (resolve, reject) => {
            ...
            this.logger.error('AmqpChannel#publish: failed with : ' + e);
    ...
    async flushWaitQueue() {
        if (this.useWaitQueue) {
            this.logger.warn('AmqpChannel: Tried to flush waitQueue on a blocked connection. Trying to write pending messages to logstash.');
