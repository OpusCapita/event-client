...
class AmqpChannel
{
    ...
    async _registerMissingConsumers()
    {
        ...
        const activeTags = [ ].concat(...(await Promise.all(topics.map(t => this._getQueueData(t).then(res => res.body.consumer_details.map(d => d.consumer_tag)).catch(e => { this.logger.error(e); return [ ]; })))));
