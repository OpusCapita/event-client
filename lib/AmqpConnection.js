...
class AmqpConnection
{
    ...
    async connect()
    {
        ...
        if (this.connectTryCount < this.config.maxConnectTryCount) {
            try {
                ...
                // FIXME this block does not catch  the Promise#retry
    ...
    async close() {
        if (this.connection && this.connectionState <= AmqpConnection.CS_BLOCKED) {
            try {
    ...
    async reconnect()
    {
        if (this.connectionState !== AmqpConnection.CS_CONNECTING) {
            await this.close();
            return this.connect();
        } else {
            return true;
