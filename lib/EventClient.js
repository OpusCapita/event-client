...
class EventClient
{
    ...
    init()
    {
        return Promise.resolve((async () =>
        {
            ...
            this.channel = new AmqpChannel(this.connection, this.logger);

            await this.connection.connect();

            this.channel.createExchange(this.exchangeName);
            this.channel.createDeadExchange(this.exchangeName);
        })());
    }
    ...
    emit(topic, message, context = null, opts = { })
    {
        return Promise.resolve((async () =>
        {
            ...
            const result = channel.publish({ exchangeName, topic, messageBuffer, options });
    ...
    getMessage(topic, autoAck = true)
    {
        ...
        {
            const channel = await this._getChannel();
            const message = await channel.getMessage(this.getQueueName(topic), autoAck);
    ...
    ackMessage(message)
    {
        ...
        {
            const channel = await this._getChannel();
            await channel.ackMessage({ fields : { deliveryTag : message.tag } });
        })());
    }
    ...
    nackMessage(message)
    {
        ...
        {
            const channel = await this._getChannel();
            await channel.nackMessage({ fields : { deliveryTag : message.tag } });
        })());
    }
    ...
    {
        return Promise.resolve((async () => (await this._getChannel()).exchangeExists(exchangeName))());
    }
    ...
    {
        return Promise.resolve((async () => (await this._getChannel()).queueExists(queueName))());
    }
    ...
    {
        return Promise.resolve((async () => (await this._getChannel()).deleteQueue(queueName, opts))());
    }
    ...
    unsubscribe(topic)
    {
        return Promise.resolve((async () => (await this._getChannel()).removeConsumer(topic))());
    }
    ...
    {
        return Promise.resolve((async () => (await this._getChannel()).consumerExists(topic))());
    }
    ...
    dispose()
    {
        return Promise.resolve((async () =>
        {
            const hadChannelOrConnection = (this.channel || this.connection) ? true : false;
    ...
    async _doReconnect()
    {
        ...
        this.connection = new AmqpConnection({ host, port, username, password, maxConnectTryCount : 10 }, this.logger);
        await this.connection.connect();

        this.channel.connection = this.connection;
        await this.channel.reRegisterConsumers();
