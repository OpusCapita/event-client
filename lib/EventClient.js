...
class EventClient
{
    ...
    init()
    {
        return Promise.resolve((async () =>
        {
            ...
            this.connection = new AmqpConnection(this.connectionConfig, this.logger);
            await this.connection.connect();

            const channel = new AmqpChannel(this.connection, this.logger, this.connectionConfig, {
            ...
            this.getChannel = new AmqpChannel(this.connection, this.logger, this.connectionConfig, {exchangeName: this.exchangeName});
    ...
    emit(topic, message, context = null, opts = { })
    {
        return Promise.resolve((async () =>
        {
            ...
            let result = false;
            try {
                result = await this.pubChannel.publish({exchangeName, topic, messageBuffer, options});
            } catch (e) {
    ...
    subscribe(topic, callback = null, opts = { })
    {
        return Promise.resolve((async () =>
        {
            ...
            if (callback) {
                ...
                await channel.registerConsumer({
                    exchangeName,
                    queueName,
                    topic,
                    prefetchCount,
    ...
    dispose()
    {
        return Promise.resolve((async () =>
        {
            ...
            await Promise.all(this.channels.map(c => c.close().catch(() => null)));
    ...
    async _getNewChannel()
    {
        ...
        const channel = new AmqpChannel(this.connection, this.logger, this.connectionConfig, {exchangeName: this.exchangeName});
    ...
    printStats() {
        ...
        console.table({
            'pubChannel': {
                'waitQueue': this.pubChannel.waitQueue.size,
                'unackedMsgs': this.pubChannel.unackedMsgs.size,
                'waitCounter': this.pubChannel.waitingCounter
            }
        });
    }
}
