...
describe('Main', () =>
{
    ...
    before('Init', async () =>
    {
        ...
        allMqNodes = await configService.getEndPoints(config.mqServiceName);

        await Promise.all(allMqNodes.map(node =>
        {
            return superagent.get(`http://${node.host}:6666/start`).timeout({ deadline : 4500 }).catch(e => console.log(e));
        }));
    });
    ...
    //     await sleep(1500);
    //
    //     //await superagent.get(`http://${consulOverride.host}:6666/kill`);
    //     //
    //     // await sleep(2500);
    //     //
    //     // await superagent.get(`http://${allMqNodes[1].host}:6666/start`);
    //
